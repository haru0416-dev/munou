...
pub(crate) fn dist_with_backoff(
    store: &Store,
    smoothing: &dyn Smoothing,
    params: &Params,
    ctx: &[TokenId],
    ...
    let mut backoff: Cow<'_, [(TokenId, f64)]> = if unigram.is_empty() {
        Cow::Owned(parrot_unigram(parrot))
    } else {
        Cow::Borrowed(unigram)
    };
    ...
    if sparse && params.lambda_cache > 0.0 && !ctx.is_empty() {
        ...
        backoff = Cow::Owned(mix_lambda(&backoff, &extra, params.lambda_cache));
    }

    if backoff.is_empty() {
        backoff = Cow::Owned(parrot_unigram(parrot));
    }
    if backoff.is_empty() {
...
pub(crate) fn counts_to_ml(counts: &[(TokenId, u32)], total: u32) -> Vec<(TokenId, f64)> {
    if total == 0 {
        return Vec::new();
    }
    let t = total as f64;
...
pub(crate) fn parrot_unigram(parrot: &[TokenId]) -> Vec<(TokenId, f64)> {
    if parrot.is_empty() {
        return Vec::new();
    }
