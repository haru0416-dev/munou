...
v0.1 の約束（CLI、閉じたマルコフ生成、説明可能な選択、会話ログから育つ）は満たしている。v0.1.1 で候補は XOR ではなくプール。v0.1.2 でライブ吸収は `p_learn`。v0.1.3 で観察窓が本線（アダプタは劣後）。v0.1.4 で次数補間・Witten-Bell・帯域ヒンジ・Boltzmann slip・連続部分列の丸暗記検出。設計 §2.2 の速度・常駐メモリは **release 実測で要件内**。mmap コールドスタートとホットパスのヒープゼロは未実装で、検証コマンドでは SKIP としている。
...
| `cargo test --workspace` | PASS（unit 33 + spec 20） |
...
| mean ctx_len_used (Markov) | 0.25 | 0.50 | 補間後のサポート次数。検索行は除外 |
| mean_sim | 0.490 | 0.341 | 帯域ヒンジで高すぎる類似を抑える |
| band_hit | — | 75% | 8本中6本 |
| rote_lcs | — | 0.69 | 連続部分列。検索が勝つと既存発話に寄る |
| slip | — | 0% | p_slip=0 |
