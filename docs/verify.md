...
同等物調査で解析した実売の VRChat 音声ペット（返答部は非LLMの確率テーブル型無脳）から、機構だけを
munou の三法に通る形へ翻訳して実装した。詳細は design.md の v0.1.15 節。新モジュール 4 つ
...
- 合いの手はログに書かない表示専用の一拍（吸収もしない）。転写 diff を取る際は text 行のみ比較すること
