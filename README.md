...
既製の LLM・API・配布重みを使わず、理解せずに会話が成立する対話プログラム。賢さではなく次の四つを目標にする。製品の本線は会話そのものより、**育つ様子を観察すること**。

**ブラウザで遊ぶ: <https://haru0416-dev.github.io/munou/>** — エンジンを WebAssembly にしたもの。サーバなし、ログは端末の localStorage のみ。

ブラウザ版では「会話のようす」で学習の状況、「これまでの記録」で覚えた言葉や節目、「返答の理由」で候補と選択結果を確認できる。「この返しが好き」「別の返しがいい」は、返し方の選ばれやすさを調整する。

1. **説明可能** — 応答は生成チェーンと選択スコアで追跡できる (`/why`)
2. **育てられる** — 語彙・遷移・記憶はユーザー可視。会話ログは毎回残り、コーパスへの吸収は対話中にランダム (`p_learn`)。ゲージは `munou observe` / `/observe`
3. **閉じている** — 外部サービスなし。知っていることは自分の会話ログと、自分で食べさせたテキスト由来
4. **ズレる** — 文脈からの適度な逸脱は仕様 (`p_slip`)

生成は可変長マルコフ（接尾辞配列の上で次数を補間）、検索は MMR、選択は閉じたハッシュ embedding に帯域ヒンジ。生成用言語モデルは無い。`--smoothing kn` は modified Kneser-Ney（既製 LM ではない）。疎な文脈だけ skip-gram と recency cache を混ぜる。既定はこれらの候補をプールして一本を選ぶ（`--mix exclusive` で v0.1 の XOR に戻せる）。会話ログは毎回残し、コーパス（SA・検索・トークナイザ）への吸収は対話中にランダム（`--p-learn`、既定 0.35）。
...
既製の LLM は使わない。モデルハブやチャット API も使わない。温度・nucleus・経路ゲート・好みラベルは、閉じたマルコフと選択器の上に**概念だけ**写したもの。

**アダプタ（伺か SHIORI / Misskey / 常駐）は後段。** 現在は CLI とブラウザ版を用意している。
